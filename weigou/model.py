...
from weigou.parallel import context
...
def init_weights(tensor: torch.Tensor):
    if tensor.dim() < 2:
        # fallback for 1D tensors (biases) or handle as needed
...
def get_cos_sin(seq_length: int, head_dim: int, base: float = 500000.0) -> Tuple[torch.Tensor, torch.Tensor]:
    assert head_dim%2==0
...
class TritonRMSNorm(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.Tensor, residual=None, dropout_p=0.0, prenorm=False, residual_in_fp32=False, return_dropout_mask=False
    ):
...
# Select RMSNorm implementation based on environment
RMSNorm = TritonRMSNorm if os.getenv('FLASH_ATTEN', '1') == '1' else LlamaRMSNorm
...
class Attention(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        assert config.num_attention_heads % tp_size == 0, "num_attention_heads should be divisible by tp world size"
        assert config.num_key_value_heads % tp_size == 0, "num_key_value_heads should be divisible by tp world size"
    ...
    def reset_parameters(self):
        init_weights(self.q_proj.weight)
        init_weights(self.k_proj.weight)
        init_weights(self.v_proj.weight)
        init_weights(self.out_proj.weight)

    def forward(self, x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor, attention_mask: Optional[torch.Tensor] = None, position_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # Dispatch attention mechanism
        if os.getenv('CONTEXT_PARALLEL', '0') == '1':
            sm_scale = 1.0 / (q.size(-1) ** 0.5)
            out = context.ring_attention(q, k, v, sm_scale, causal).transpose(1, 2)
...
class MLP(nn.Module):
    ...
    def reset_parameters(self):
        init_weights(self.up_proj.weight)
        init_weights(self.gate_proj.weight)
        init_weights(self.down_proj.weight)
...
class DecoderLayer(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        self.cos, self.sin = context.update_rope_for_context_parallel(self.cos, self.sin)

    def forward(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None, position_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
...
class Llama(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: Optional[torch.Tensor] = None, position_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
