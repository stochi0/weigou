...
def download_model(model_name: str, hf_token: str, out_dir: str = MODELS_DIR) -> str:
    """Download HF model weights to out_dir. Returns path to downloaded model."""
    dst = os.path.join(out_dir, model_name.replace("/", "__"))
    os.makedirs(dst, exist_ok=True)

    if os.path.exists(os.path.join(dst, "config.json")) and glob.glob(os.path.join(dst, "*.safetensors")):
        print(f"Model {model_name} already at {dst}")
        return dst
    ...
    if not glob.glob(os.path.join(dst, "*.safetensors")):
        raise ValueError(f"Model {model_name} has no safetensors files.")
